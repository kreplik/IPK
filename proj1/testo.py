...
class ExecutableTester:
    ...
    def setup(self, args=["-t", "udp", "-s", "localhost", "-p", "4568"]):
        if self.process:
            self.teardown()
        self.stdout_queue = queue.Queue()
        self.stderr_queue = queue.Queue()
...
def all_args(tester):
    """Test that the program exits with a non-zero exit code when the -s argument is not provided."""
    tester.setup(args=["-t", "udp", "-s", "localhost", "-p", "4568"])
    tester.send_eof()
...
def tcp_hello(tester):
    tester.start_server("tcp", 4568)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4568"])
